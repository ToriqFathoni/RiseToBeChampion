...
public class HeavyAttackCommand implements Command {
    ...
    public HeavyAttackCommand(Combatant actor, Combatant target, int damage, CombatLogger logger) {
        this.actor = actor;
        this.target = target;
        this.damage = damage;
        this.logger = logger;
    ...
    @Override
    public void execute() {
        actor.performAction(target, EntityState.ATTACK_HEAVY, damage, logger);
    }
}
